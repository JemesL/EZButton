...
//        test()
    ...
    func addBtn() {
        ...
        btn.distribution = .topIconBottomTitle
        
        btn.alignemnt = .rightBottom
        
//        btn.offset = 3
        btn.edge = UIEdgeInsetsMake(10, 10, 10, 10)
        btn.text = "123333"
        btn.image = UIImage(named: "tag_icon_orange")
        btn.imageSize = CGSize(width: 40, height: 40)
        btn.snp.makeConstraints { make in
            make.centerX.centerY.equalToSuperview()
//            make.size.equalTo(CGSize(width: 240, height: 150))
        }
//        btn.frame = CGRect(x: 110, y: 110, width: 100, height: 100)
    }
